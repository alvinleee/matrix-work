...
func New(eth Backend) (*BlockGenor, error) {
	if nil == &eth {
		log.Error("nil == &eth Error")
		return nil, ParaNull
	}
	if nil == eth.BlockChain().Engine() {
		log.Error("eth.BlockChain().Engine() Error")
		return nil, ParaNull
	}
	//if nil==eth.ReElection(){
	//	return nil,ParaNull
	//}
