...
func (b *LesApiBackend) SignTx(signedTx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	account := accounts.Account{Address: b.eth.config.Etherbase}
...
func (b *LesApiBackend) SendBroadTx(ctx context.Context, signedTx *types.Transaction, bType bool) error {
	return nil //b.eth.txPool.AddBroadTx(signedTx,bType)
}
...
func (b *LesApiBackend) FetcherNotify(hash common.Hash, number uint64) {
	...
	//for _,id := range ids{
	//peer := b.eth.protocolManager.peers.Peer(id.String())
	//b.eth.protocolManager.fetcher.Notify(id.String(), hash, number, time, peer.RequestOneHeader, peer.RequestBodies)
	//}
}
