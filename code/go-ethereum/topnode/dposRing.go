...
import (
	...
	"github.com/ethereum/go-ethereum/messageState"
)
...
func (ds *DPosVoteState) addVote(vote *mc.HD_ConsensusVote) (interface{}, []voteInfo) {
	...
	for _, item := range ds.AffirmativeVotes {
		if item.hash == insVote.hash {
			log.Error("topnodeOnline", "添加投票 vote", vote, "item", item.data)
			log.Error("topnodeOnline", "添加投票 vote:insVote.hash", insVote.hash, "item.hash", item.hash)
			return nil, nil
		}
	}
