...
func (self *controller) run() {
	...
	if self.number == 1 && self.curTurns == 0 {
		self.timer = time.NewTimer(man.LRSWaitingBlockReqTimer + 60*time.Second)
	} else {
		self.timer = time.NewTimer(man.LRSWaitingBlockReqTimer)
	}
...
func (self *controller) timeoutHandle() error {
	...
	self.curLeader.Set(leaders.leader)
	self.curTurns = leaders.turns
	self.timer = time.NewTimer(man.LRSReelectLeaderTimer)
...
func (self *controller) stateChangeToWaitingDPOSResult() {
	...
	if self.number == 1 && self.curTurns == 0 {
		self.timer = time.NewTimer(man.LRSWaitingDPOSResultTimer + 60*time.Second)
	} else {
		self.timer = time.NewTimer(man.LRSWaitingDPOSResultTimer)
	}
