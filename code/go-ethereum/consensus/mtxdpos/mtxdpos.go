...
func (md *MtxDPOS) ResetChainReader(chain consensus.ChainReader) {
	md.chain = chain
}
...
func (md *MtxDPOS) VerifyBlocks(headers []*types.Header) error {
	if len(headers) <= 0 {
	...
	for _, header := range headers {
		if nil == preGraph {
			preGraph, err = md.getValidatorGraph(header.Number.Uint64())
			if err != nil {
				return err
			}
		}
...
func (md *MtxDPOS) VerifyStocksWithNumber(validators []common.Address, number uint64) bool{
...
func (md *MtxDPOS) getValidatorStocks(number uint64) (map[common.Address]uint16, error) {
	graphInfo, err := md.getValidatorGraph(number)
	if err != nil {
		return nil, err
	}
...
func (md *MtxDPOS) getValidatorGraph(number uint64) (*mc.TopologyGraph, error) {
	parentNumber := number
	if parentNumber != 0 {
		parentNumber--
	}
