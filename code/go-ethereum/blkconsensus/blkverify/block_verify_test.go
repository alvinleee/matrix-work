...
func TestRightRequestMsg_01(t *testing.T) {
	eth := newEth(t, nil, false)

	reElection, err := reelection.New(eth.BlockChain(), nil)
	if err != nil {
		t.Fatalf("create reelection server err!")
	}

	blkVerify, err := NewBlockVerify(eth)
	if err != nil {
		t.Fatalf("StartServer failed: %v\n", err)
	}
	...
	requstMsg := &mc.BlockVerifyConsensusReq{
		Header:  createHeader(t, eth.BlockChain(), leaderMsg.Leader),
		TxsCode: nil,
	}
...
func newEth(t *testing.T, confOverride func(*eth.Config), isBroadcastNode bool) *eth.Ethereum {
